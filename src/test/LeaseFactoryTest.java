...
class LeaseFactoryTest {
    ...
    @Test
    void testCreateTermsAndConditionsSuccess() {
        TermsAndConditions terms = LeaseFactory.createTermsAndConditions(
                "TC001", "L001", "Tenant must not sublet the property.", true);
        assertNotNull(terms);
        assertEquals("TC001", terms.getTermsId());
        assertEquals("L001", terms.getLeaseId());
        assertEquals("Tenant must not sublet the property.", terms.getDescription());
        assertTrue(terms.isAccepted());
        System.out.println("TermsAndConditions created successfully: " + terms);
    }

    @Test
    void testCreateTermsAndConditionsFailure() {
        TermsAndConditions terms = LeaseFactory.createTermsAndConditions(
                "", "L001", "Tenant must not sublet the property.", true);
        assertNull(terms);
        System.out.println("Failed to create TermsAndConditions with empty termsId as expected.");
    }
}
